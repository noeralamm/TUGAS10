...
$cases = mysqli_query($koneksi,"select * from db_covid"); //mengambil data dari data base
while($row = mysqli_fetch_array($cases)){
	$negara[] = $row['country'];
	$query = mysqli_query($koneksi,"SELECT sum(total_cases) as total_cases from db_covid where id ='".$row['id']."'");
		...
		var config = {
			...
			data: {
				datasets: [{
					...
					backgroundColor: [ //penggunaan sourcecode warna chart
					'rgba(255, 99, 132, 0.2)',
					'rgba(54, 162, 235, 0.2)',
					'rgba(255, 206, 86, 0.2)',
					'rgba(75, 192, 192, 0.2)',
